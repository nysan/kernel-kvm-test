...
# /usr/bin/kvm   -hda /media/sdb5/eel/build/tmp/deploy/images/core-image-minimal-qemux86-64.ext3 -machine accel=kvm:tcg -kernel arch/x86/boot/bzImage -nographic -smp 4 -append " root=/dev/sda console=ttyS0 earlyprintk=serial"
...
on your root fs with \"-r / -a init=/bin/bash\"
 ...
 -e, --hda image
 Add the hda file as disk into the VM.
...
GETOPT=`getopt -o a:d:e:Dhbr:sS:v --long append:,disk:ext:,no-gdb,help,boot,root:,sdl,smp:,vnc \
 -n "$(basename \"$0\")" -- "$@"`
...
while true; do
 ...
 -e|--ext)
 TDIR=`mktemp -d`
 mkdir -p $TDIR/lib/modules
# INSTALL_MOD_PATH=$TDIR make modules_install firmware_install
 QEMU_OPTIONS="$QEMU_OPTIONS -hda $2"
 QEMU_OPTIONS="$QEMU_OPTIONS \
-virtfs local,id=home,path=/home,security_model=mapped-xattr,mount_tag=home \
-device virtio-9p-pci,fsdev=home,mount_tag=home \
-virtfs local,id=modules,path=$TDIR/lib/modules,security_model=mapped-xattr,mount_tag=modules \
-device virtio-9p-pci,fsdev=modules,mount_tag=modules"
 KERNEL_APPEND2="$KERNEL_APPEND2 root=/dev/sda rw "
 USE_DISK=1
 shift
 ;; 
 ...
 -b|--boot)
 TDIR=`mktemp -d`
 fakeroot debootstrap --arch amd64 saucy $TDIR
 #sudo INSTALL_MOD_PATH=$TDIR make modules_install
 #sudo INSTALL_MOD_PATH=$TDIR make firmware_install
 echo "\
#!/bin/sh \
...
exec /bin/sh -i" > $TDIR/init
 chmod 777 $TDIR/init
 echo "$TDIR Bootstrapped" 
 exit 0
 ;;
 -s|--sdl)
 USE_SDL=1
